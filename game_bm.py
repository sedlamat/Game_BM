...
class BigMoney:
    ...
    def play(self):
        ...
        while time.time() - t_start < 300: # play for 5 minutes
            if game_situation_flag == 0:
                ...
                print "time reloading", time.time() - t0
                ...
                if not last_click_successful:
                    ...
                    t17 = time.time()
                    print "check for money drop", t17 - t16
            t18 = time.time()     
            self.image_buffer.append(self.area_img_BGR)
            self.time_buffer.append(time.time()-t_start)
            t19 = time.time()
            print "append images and time for checkups", t19 - t18
    ...
    def get_best_tuple(self, tuple_positions):
        ...
        POWER_VAL = max(1,max_column_height-4)
        print max_column_height, POWER_VAL
        # if 5 <= max_column_height <= 6:
        #     POWER_VAL = 2
        # elif 7 <= max_column_height <= 8:
        #     POWER_VAL = 3
        # elif 9 <= max_column_height <= 10:
        #     POWER_VAL = 4
        # elif 11 <= max_column_height <= 12:
        #     POWER_VAL = 5
        ...
        if max_column_height < 10:
            tuples_values = sorted(tuples_values, key=lambda values:(-values[6], # any bag collected
                                                                     values[7],
                                                                     values[4],
                                                                     values[1]))
